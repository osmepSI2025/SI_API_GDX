...
public class MOrganizationJuristicPersonService 
{
    ...
    public async Task<IActionResult> BatchEndOfDay_MOrganizationJuristicPerson(searchMOrganizationJuristicPersonModels xmodel)
    {
        ...
        if (JuristicPersonApiResponse.data?.OrganizationJuristicPerson != null)
        {
            var item = JuristicPersonApiResponse.data.OrganizationJuristicPerson;
            try
            {
            ...
            new TOrganizationJuristicAddress
            {
               ...
               : new List<TOrganizationJuristicAddress>(),
                ...
                else
                {
                    ...
                    // TJuristicObjectives
                    existing.TJuristicObjectives.Clear();
                    if (item.OrganizationJuristicObjective != null)
                    {
                        foreach (var obj in item.OrganizationJuristicObjective)
                        {
                            existing.TJuristicObjectives.Add(new TJuristicObjective
                            {
                                OrganizationJuristicId = item.OrganizationJuristicID,
                                JuristicObjective = obj.JuristicObjective,
                                JuristicObjectiveCode = obj.JuristicObjectiveCode,
                    ...
                    // TJuristicPersonLists
                    existing.TJuristicPersonLists.Clear();
                    if (item.OrganizationJuristicPersonList != null)
                    {
                        foreach (var p in item.OrganizationJuristicPersonList)
                        {
                            existing.TJuristicPersonLists.Add(new TJuristicPersonList
                            {
                                OrganizationJuristicId = item.OrganizationJuristicID,
                                JuristicPersonSequence = p.JuristicPersonSequence,
                                JuristicPersonType = p.JuristicPersonType,
                    ...
                    // TOrganizationJuristicAddresses
                    existing.TOrganizationJuristicAddresses.Clear();
                    if (item.OrganizationJuristicAddress?.AddressType != null)
                    {
                        existing.TOrganizationJuristicAddresses.Add(new TOrganizationJuristicAddress
                        {
                            OrganizationJuristicId = item.OrganizationJuristicID,
                            Address = item.OrganizationJuristicAddress.AddressType.Address,
                            Building = item.OrganizationJuristicAddress.AddressType.Building,
                    ...
                    // TOrganizationJuristicPersonDescriptions
                    existing.TOrganizationJuristicPersonDescriptions.Clear();
                    if (item.OrganizationJuristicPersonDescription != null)
                    {
                        foreach (var d in item.OrganizationJuristicPersonDescription)
                        {
                            existing.TOrganizationJuristicPersonDescriptions.Add(new TOrganizationJuristicPersonDescription
                            {
                                OrganizationJuristicId = item.OrganizationJuristicID,
                                DescriptionSequence = d.OrganizationJuristicPersonDescriptionSequence,
                                DescriptionType = d.OrganizationJuristicPersonDescriptionType,
